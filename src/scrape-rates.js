...
async function extractCloudbedsByVisibleLines(page, roomAliasesByTarget) {
  await page.waitForLoadState('networkidle', { timeout: 45000 }).catch(() => {});
  await closePopups(page);
  ...
  const priceRegex = /(?:MXN|MEX\$|\$)?\s*[0-9]{1,3}(?:[.,][0-9]{3})*(?:[.,]\d{2})?\s*(?:MXN|mxn|MEX\$)?|(?:MXN|mxn|MEX\$|\$)\s*[0-9]+/gi;
  ...
  function isAnotherRoomTitle(line, currentAliases) {
    ...
    if (current.some((a) => a.length >= 6 && nLine.includes(a))) return false;
  ...
  for (const [targetRoom, aliases] of Object.entries(roomAliasesByTarget)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (let j = i; j < Math.min(lines.length, i + 90); j++) {
        ...
        // Evita falsos positivos: teléfonos, IDs, impuestos aislados, textos sin moneda.
        if (!/(MXN|mxn|MEX\$|\$)/i.test(lines[j])) continue;
...
async function scrapeHotelMarielena(page, checkIn, checkOut) {
  ...
  const extracted = CONFIG.hotels.hm.engine === 'cloudbeds'
    ? await extractCloudbedsByVisibleLines(page, aliases)
    : await extractPricesByAliases(page, aliases);
